...
public class ArticleListController extends AdminBaseController {
	...
	public String listArticle(@ModelAttribute Article article, Model model, RedirectAttributes reat, Principal principal, HttpServletRequest request) {
		...
		try {
			...
			request.getSession().setAttribute("adminSearch_article", article);
	...
	public String listArticle(Model model, RedirectAttributes reat, Principal principal, HttpServletRequest request) {
		...
		try {
			...
			Article obj = (Article) request.getSession().getAttribute("adminSearch_article");
			...
			request.getSession().setAttribute("adminSearch_article", obj);
			...
			request.getSession().setAttribute("listArticleAdmin_pageNumber", pageNumber);
			request.getSession().setAttribute("listArticleAdmin_totalPages", totalPages);
	...
	public String listArticle(@PathVariable String whichPage, Model model, Principal principal, HttpServletRequest request) {
		...
		try {
			int pageNumber = (int) request.getSession().getAttribute("listArticleAdmin_pageNumber");
			int totalPages = (int) request.getSession().getAttribute("listArticleAdmin_totalPages");
			
			if ("previous".equals(whichPage)) {
				if (pageNumber == 0) return "redirect:/admin/article/list";
				else {
					pageNumber--; 
				}
