...
public class HomeGalleryListController extends HomeBaseController {
	...
	public String listGallery(@ModelAttribute Gallery gallery, Model model, RedirectAttributes reat, Principal principal, HttpServletRequest request) {
		...
		try {
			...
			request.getSession().setAttribute("homeSearch_gallery", gallery);
				
			return "redirect:/home/gallery/list";
		} catch (Exception e) {
	...
	public String listGallery(Model model, RedirectAttributes reat, Principal principal, HttpServletRequest request) {
		...
		try {
			...
			Gallery obj = (Gallery) request.getSession().getAttribute("homeSearch_gallery");
			...
			request.getSession().setAttribute("listGalleryHome_pageNumber", pageNumber);
			request.getSession().setAttribute("listGalleryHome_totalPages", totalPages);
	...
	public String listGallery(@PathVariable String whichPage, Model model, Principal principal, HttpServletRequest request) {
		...
		try {
			int pageNumber = (int) request.getSession().getAttribute("listGalleryHome_pageNumber");
			int totalPages = (int) request.getSession().getAttribute("listGalleryHome_totalPages");
			
			if ("previous".equals(whichPage)) {
				if (pageNumber == 0) return "redirect:/home/gallery/list";
				else {
					pageNumber--; 
				}
