...
import org.pf.school.validator.accounts.PaymentValidator;
...
@Controller
@RequestMapping(value="/accounts/student/issue/payment")
public class StudentPaymentController extends AccountsBaseController {
	...
	public String receivePayment(@PathVariable UUID id, Model model,
		...
		expenditure.setMember(student.getMember());
		
		expenditure.setPaidTo(student.getLabel());
		
		model.addAttribute("expenditure", expenditure);
		model.addAttribute("student", student);
		
		return "accounts/student/payment";
	}
	
	@PostMapping("/*")
	public String receivePayment(@ModelAttribute Expenditure expenditure,
			BindingResult result, Model model, RedirectAttributes reat,
			Principal principal) {
		
		if (expenditure.getMember()==null) {
			reat.addFlashAttribute("message", "Student record is not found.");
			return "redirect:/accounts/student/list/current";
		}
	
		this.paymentValidator.validate(expenditure, result);
		
		Student student = (Student) this.studentRepo.findByMember(expenditure.getMember());
		...
		try {
			...
			if (tr == null) {
				...
				if (tr.isStatus()) {
					reat.addFlashAttribute("message", "Payment released successfully.");
					return "redirect:/accounts/student/view/" + student.getId();
				} else {
